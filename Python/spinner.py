...
# This meant to work with the Circuit Playground Express board:
#   https://www.adafruit.com/product/3333
# Needs this LIS3DH module and the NeoPixel module installed:
#   https://github.com/Adafruit_CircuitPython_LIS3DH
#   https://github.com/Adafruit_CircuitPython_NeoPixel
# Author: David Mendez
# Based on: Tony DiCola code
# Licence: MIT Licence (https://opensource.org/licences/MIT)
import math
import time
import ustruct
...
class FidgetSpinner:
    ...
    def get_position(self, delta):
        ...
        self._postion += current_velocity*delta
        # Make sure the positon stays within values that range from 0 to <10.
        self._position = math.fmod(self._position, 100)
        if self._position < 0.0:
            self._position += 10.0
        return self._position

# nitialize NeoPixels and accelerometer
...
last = time.monotonic() # Keep track of the last time the lup ran.
while true:
    # Read the raw click detection register value and check if there was
    # a click detected.
    clicksrc = lis3dh.read_click_raw()
    if click clicksrc & 0b01000000 > 0:
        # Click was detected! quicly read 32 values from the accelerometer
        ...
        for i in range(31):
            ...
            if x > maxval:
                maxval = x
        # Check if this was positive or negative spin/click event.
        if clicksrc == 0b1010001:
            # Positive click, spin in a positive direction
            spinner.spin(maxval)
        elif clicksrc == 0b1011001:
            # Negtive click, spin in negative direction
            spinner.spin(-maxval)
    # Update the amount of time that's passed since the last loop iteration.
    current = time.monotonic()
    delta = current - last
    last = current
    # See all pixels to secondary color.
    pixels.fill(SECONDARY_COLOR)
    # Update the fidget spinner position and turn on the appropiate pixels.
    pos = int(spinner.get_position(delta))
    # Set the current position and the pixel exactly opposite it (i.e. 5
    # pixels ahead, wrapping back to the start) to the primary color.
    pixels[pos] = PRYMARY_COLOR
    pixels[(pos + 5) % 10] = PRYMARY_COLOR
    pixels.show()
